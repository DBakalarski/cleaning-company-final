...
/**
 * Before/after card. Shows the "przed" photo by default; the "po" photo
    ...
    <figure className="m-0 flex flex-col gap-3" data-reveal-item>
      <button
        ...
        aria-label={`${pair.caption}: pokaż efekt przed i po (${
          revealed ? "widoczne: po" : "widoczne: przed"
        })`}
      ...
      >
        ...
        />
        {/* After (po) — fades in on hover / when revealed */}
        <Image
          src={pair.after}
          alt=""
          aria-hidden="true"
        ...
        {/* "Przed" badge — hidden once "po" is shown */}
        <span
          className={`absolute left-3 top-3 rounded-full bg-ink/85 font-heading font-semibold uppercase text-white backdrop-blur-sm transition-opacity duration-300 group-hover:opacity-0 ${badgeClass} ${
            revealed ? "opacity-0" : "opacity-100"
          }`}
        >
          Przed
        </span>
        {/* "Po" badge — shown once "po" is visible */}
        <span
          className={`absolute left-3 top-3 rounded-full bg-accent font-heading font-semibold uppercase text-ink transition-opacity duration-300 group-hover:opacity-100 ${badgeClass} ${
            revealed ? "opacity-100" : "opacity-0"
          }`}
        >
          Po
